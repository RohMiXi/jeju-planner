...
import { Loader2, Plus, MapPin, Calendar, Clock, X, Search } from "lucide-react"
...
export function AddScheduleDrawer({ isOpen, onOpenChange, onSuccess }: AddScheduleDrawerProps) {
    const { user } = useAuth()
    ...
    const resetForms = () => {
        setMode("select")
        setSuggestForm({ category: "식당", name: "", address: "", naver_map_url: "", description: "" })
        setAddForm({ day_number: "1", start_time: "", end_time: "", location: "", address: "", purpose: "", remarks: "", lat: null, lng: null })
        setSearchQuery("")
        setSearchResults([])
        setIsLoading(false)
    }
    ...
    const handleAddSubmit = async (e: React.FormEvent) => {
        ...
        if ((!finalLat || !finalLng) && addForm.address) {
            console.log("Coordinates missing at save time. Fetching for address:", addForm.address)
            const coords = await fetchGeocode(addForm.address)
            if (coords) {
                finalLat = coords.lat
                finalLng = coords.lng
                console.log("Fetched Coordinates:", finalLat, finalLng)
            } else {
                console.warn("FAILED to fetch coordinates for:", addForm.address)
            ...
            <Drawer.Portal>
                ...
                <Drawer.Content className="bg-[#F7F3F2] flex flex-col rounded-t-[20px] h-[90vh] mt-24 fixed bottom-0 left-0 right-0 z-[60] focus:outline-none">
                    ...
                    <div className="p-4 bg-white rounded-t-[20px] shadow-sm z-10 relative">
                        ...
                        <button onClick={handleClose} className="absolute right-4 top-4 p-2 bg-gray-100 rounded-full hover:bg-gray-200">
                            <X className="w-5 h-5 text-gray-500" />
                        </button>

                        <Drawer.Title className="font-bold text-xl text-[#1D1D1F] text-center">
